...
(function (global) {
  ...
  function _radialDisplacements(fn, bezel, refraction) {
    // 位移强度分布：正弦钟形，边缘 0 → bezel 中部峰值 → 内缘 0。
    // 原实现 mags[i] = r.x * fn(t) 会把折射压成极窄尖峰（只占元素 2-7%），
    // 导致折射带肉眼不可见。钟形分布让折射覆盖整个 bezel 宽度。
    const N = 128;
    const mags = new Float32Array(N);
    for (let i = 0; i < N; i++) {
      const t = i / (N - 1);
      mags[i] = Math.sin(t * Math.PI);
    }
